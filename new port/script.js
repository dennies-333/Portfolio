let w, h;
let bgColor = "#000"; // Dark mode background
let circleColor = "#fff"; // Dark
...
requestAnimationFrame(function anim(t) {
    ...
    drawCircle(0, 0, w * 10);
    ctx.fillStyle = ctx.strokeStyle = circleColor;
    t/=1000
...
window.onload = function() {
    document.querySelector('.moon-icon').classList.add('animate-moon');  // Add animation on page load
  };

document.querySelector('.moon-icon').addEventListener('animationend', function() {
document.querySelector('.moon-icon').classList.remove('animate-moon');  // Remove animation after it ends
});
...
document.querySelector('.moon-icon').addEventListener('click', function() {
    ...
    if (document.body.classList.contains('light-mode')) {
        ...
        bgColor = "#fff"; // Dark mode background
        circleColor = "#000"; // Dark mode circle and line color (you can change this)
    } else {
        bgColor = "#000"; // Light mode background
        circleColor = "#fff"; // Light mode circle and line color (you can change this)
    }
  });
  ...
  document.querySelector('.sun-icon').addEventListener('click', function() {
    ...
    if (document.body.classList.contains('light-mode')) {
        bgColor = "#fff"; // Light mode background
        circleColor = "#000"; // Light mode circle and line color
    } else {
        bgColor = "#000"; // Dark mode background
        circleColor = "#fff"; // Dark mode circle and line color
    }
  });